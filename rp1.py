...
def rp1():
	...
	rows = \
		[[['x_u_'+str(m)+str(k) for k in xrange(len(K))]+['x_cc_'+str(m)+str(j) for j in filter(lambda x: x!= m,xrange(len(J)))]+['x_cc_'+str(j)+str(m) for j in filter(lambda x: x!= m,xrange(len(J)))]+['x_c_'+str(i)+str(m) for i in xrange(len(I))]+['e_c_'+str(m)],\
		[T*(w+v*(Dist[I[J[m].index].index][I[K[k].index].index]**2)) for k in xrange(len(K))]+[T*(w+v*(Dist[I[J[m].index].index][I[J[j].index].index]**2)) for j in filter(lambda x: x!= m,xrange(len(J)))]+[T*w  for j in filter(lambda x: x!= m,xrange(len(J)))]+[T*(w+c*s)*I[i].R for i in xrange(len(I))]+[-1]] for m in xrange(len(J))]+\
		...
		[[['x_u_'+str(m)+str(k) for k in xrange(len(K))]+['x_cc_'+str(m)+str(j) for j in filter(lambda x: x!= m,xrange(len(J)))]+['x_cc_'+str(j)+str(m) for j in filter(lambda x: x!= m,xrange(len(J)))]+['x_c_'+str(i)+str(m) for i in xrange(len(I))],\
		[1 for k in xrange(len(K))]+[1 for j in filter(lambda x: x!= m,xrange(len(J)))]+[-1  for j in filter(lambda x: x!= m,xrange(len(J)))]+[(s-1)*I[i].R for i in xrange(len(I))]] for m in xrange(len(J))]+\
