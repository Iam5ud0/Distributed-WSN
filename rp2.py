...
def rp2():
	...
	sense= 'E'*len(J)+'E'*len(I)+'E'*len(J)+'L'*len(I)+'L'*((len(I)*len(J)) - len(J))+'L'*(len(J)**2 - len(J))+'L'*(len(K)*len(J))+'EE'+'L'*len(I)+'L'*len(J)+'L'*len(I)

	rhs=[0]*len(J)+[0]*len(I)+[0]*len(J)+[1]*len(I)+[0]*((len(I)*len(J)) - len(J))+[0]*(len(J)**2 - len(J))+[0]*(len(K)*len(J))+[C]+[S]+[I[i].E for i in xrange(len(I))]+[0]*len(J)+[(-1)*I[i].E for i in xrange(len(I))]
	#print (rows[:-37])
