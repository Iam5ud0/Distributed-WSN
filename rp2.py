...
sense= 'E'*len(J)+'E'*len(I)+'E'*len(J)+'E'*len(I)+'L'*(len(I)*len(J))+'L'*(len(J)**2)+'L'*(len(K)*len(J))+'L'*(len(K)*len(J))+'EE'+'L'*len(I)+'L'*len(J)+'L'*len(I)+'L'*len(J)

rhs=[0]*len(J)+[0]*len(I)+[0]*len(J)+[1]*len(I)+[0]*(len(I)*len(J))+[0]*(len(J)**2)+[0]*(len(K)*len(J))+[0]*(len(K)*len(J))+[C]+[S]+[I[i].E for i in xrange(len(I))]+[I[i].E for i in xrange(len(I))]+[(-1)*J[j].E for j in xrange(len(J))]
def rp2():
	try:
		prob=cplex.Cplex()
